...
resource "azurerm_consumption_budget_subscription" "this" {
  ...
  subscription_id = "/subscriptions/${coalesce(
    ...
    try(var.local_combined_resources["subscriptions"][try(var.settings.subscription.lz_key, var.client_config.landingzone_key)][var.settings.subscription.key].subscription_id, null),var.client_config.subscription_id)}"
