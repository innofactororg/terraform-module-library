module "wvd_application_groups" {
  ...
  workspace_id        = can(each.value.wvd_workspace.id) || can(each.value.workspace_id) ?  try(each.value.wvd_workspace.key, each.value.wvd_workspace_key) : local.combined_objects_wvd_workspaces[try(each.value.wvd_workspace.lz_key, local.client_config.landingzone_key)][try(each.value.wvd_workspace_key, each.value.wvd_workspace.key)].id
